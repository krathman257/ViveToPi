...
public:
	//Constructor
	Canvas(std::string dev_dir_vive, std::string dev_dir_mon, int cam_dev, std::string imagePath, std::string textPath, bool m, bool v, std::mutex *mx){
	...
	void printInstructions(std::vector<std::vector<std::string>> instructions){
		...
		for(std::vector<std::string> instruction : instructions){
			printf("%d) ", counter++);
			for(std::string command : instruction){
